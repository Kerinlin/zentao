...
| ------ | ------ | ------ | ------ |
| <a id="property-autofill"></a> `autoFill?` | `boolean` | 是否在执行 `update` 操作时自动填充未传入的字段。 设为 `true` 后，会先 GET 当前对象，把用户未显式传入（含 `params.data`）的可写字段用现值补齐，再发起 PUT，避免禅道按全量写把未提交字段覆盖为空。回填不限于 body schema：schema 未声明但对象上存在的可写标量（如 `mailto`）也会合并；只读/审计字段与复杂嵌套结构会被跳过。用户显式传入的字段始终优先。因此只需传想修改的字段即可。仅对 `type: 'update'` 且模块存在 `type: 'get'` 动作时生效。 不传时回落到全局 `autoFill`，默认 false。 | - |
