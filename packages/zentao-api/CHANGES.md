...
- `bug` 的 `create` / `update` 请求体 schema 补齐 `assignedTo`（OpenAPI 未声明，但禅道实际接受），便于显式指派与 create 写入。

## 0.4.1 - 2026-07-28

### 修复

- `autoFill` 回填不再局限于 OpenAPI body schema：从 GET 现值合并用户未显式传入的可写字段（如 `mailto`、`assignedTo`），避免禅道按全量 PUT 清空未声明字段。
- `update` 的 body 组装会保留 schema 外、已出现在 params / `params.data` 中的可写字段；`create` 仍严格按 schema，避免脏写。
- 回填时跳过只读/审计字段与复杂嵌套结构，并将 `{ account }` 用户对象规范为 account 字符串。
