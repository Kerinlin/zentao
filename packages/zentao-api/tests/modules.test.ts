...
describe('builtin overrides (override.ts)', () => {
  ...
  test.each(['create', 'update'] as const)('bug/%s requestBody includes assignedTo', (actionName) => {
    const properties = getModuleAction('bug', actionName)!.requestBody?.schema?.properties as Record<
      string,
      Record<string, unknown>
    >;
    expect(properties.assignedTo).toEqual({
      type: 'string',
...
describe('high-level request', () => {
  ...
  test('autoFill fills omitted update fields from the current object before PUT', async () => {
      ...
      if (req.method === 'GET') {
        ...
        return Response.json({
          ...
          bug: {
            ...
            execution: 0,
            story: 0,
            // schema 外可写字段：应回填，避免禅道全量 PUT 清空。
            mailto: 'qa1,qa2',
            keywords: 'login',
            assignedTo: { id: 1, account: 'admin', realname: 'Admin' },
            // 只读 / 嵌套：不应进入 PUT。
            status: 'active',
            resolvedBy: 'admin',
    ...
    try {
      ...
      // 用户显式传入的 title 保留；schema 内与 schema 外可写字段用现值补齐。
      ...
      expect(putBody).not.toHaveProperty('actions');
  ...
  test('autoFill keeps explicit params over schema-extra current values', async () => {
    let putBody: Record<string, unknown> | undefined;
    const server = createMockServer(async (req) => {
      if (req.method === 'GET') {
        return Response.json({
    ...
    try {
      ...
      await request(
        'bug/update',
        { id: 7, title: 'new title', mailto: 'new@example.com' },
        { autoFill: true },
      );
