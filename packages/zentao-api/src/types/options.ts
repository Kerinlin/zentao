...
export interface RequestOptions extends ProcessListOptions {
   ...
   *
   * 设为 `true` 后，会先 GET 当前对象，把用户未显式传入（含 `params.data`）的可写字段
   * 用现值补齐，再发起 PUT，避免禅道按全量写把未提交字段覆盖为空。
   * 回填不限于 body schema：schema 未声明但对象上存在的可写标量（如 `mailto`）也会合并；
   * 只读/审计字段与复杂嵌套结构会被跳过。用户显式传入的字段始终优先。
