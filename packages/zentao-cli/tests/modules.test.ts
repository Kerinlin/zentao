...
describe('module registry (zentao-api)', () => {
    ...
    test('bug create and update requestBody include assignedTo', () => {
        const bug = getModule('bug')!;
        for (const actionName of ['create', 'update'] as const) {
            const action = findAction(bug, actionName);
            expect(action).toBeDefined();
            const properties = action!.requestBody?.schema?.properties as
                | Record<string, { type?: string; description?: string }>
                | undefined;
            const assignedTo = properties?.assignedTo;
            expect(assignedTo).toBeDefined();
            expect(assignedTo).toMatchObject({
                type: 'string',
                description: expect.stringContaining('指派'),
            });
        }
    });
});
