...
describe('filterData', () => {
    ...
    test('filters with = operator (alias of :)', () => {
        const result = filterData(sampleData, ['status=normal']);
        expect(result.length).toBe(2);
        expect(result.every((r) => r.status === 'normal')).toBe(true);
    });

    test('= and : produce identical results', () => {
        const byColon = filterData(sampleData, ['owner.name:admin']);
        const byEq = filterData(sampleData, ['owner.name=admin']);
        expect(byEq).toEqual(byColon);
    });

    test('= does not shadow != (regression)', () => {
        // `=` 加入 OPERATORS 后必须保证 `!=` 仍优先匹配，不被误判为 field 含 `=`
        const result = filterData(sampleData, ['status!=normal']);
        expect(result.length).toBe(2);
        expect(result.every((r) => r.status !== 'normal')).toBe(true);
    });

    test('= does not shadow >= or <= (regression)', () => {
        expect(filterData(sampleData, ['priority>=3']).length).toBe(2);
        expect(filterData(sampleData, ['priority<=2']).length).toBe(2);
    });
