...
> ✅ 已实现：使用 `zentao upgrade` 检查 npm 上的最新正式版并自动升级（支持 `--yes` 跳过确认）。
> 启动时也会按间隔后台检查，有更新时在 stderr 提示执行 `zentao upgrade`。
> 业务对象更新仍走 `zentao update <module> ...` / `zentao <module> update ...`，与 CLI 自升级分离。
...
>
> 仍可增强：按模块差异化注入策略、`autoSet` 分级（off/on-empty/always）、失效对象检测。MCP workspace tools 已随精简 MCP 落地。
