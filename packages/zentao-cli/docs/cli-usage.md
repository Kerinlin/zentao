...
通过 `zentao upgrade` 检查 npm registry 上的最新正式版，并按当前安装方式（`npm` / `bun` 全局安装）自动升级。

```bash
# 检查并交互确认后升级
$ zentao upgrade

# 跳过确认，直接升级（适合脚本 / Agent）
$ zentao upgrade --yes
```

说明：

* 仅升级 `@kerin/zentao-cli` 全局包；业务对象更新请用 `zentao update <module> ...` 或 `zentao <module> update ...`
* 日常使用时 CLI 也会按间隔后台检查新版本，有更新时在 stderr 提示执行 `zentao upgrade`
* 非交互环境（无 TTY）必须加 `--yes`，否则会退出并提示手动确认
