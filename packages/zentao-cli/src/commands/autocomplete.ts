...
_${command}_completion() {
  ...
  case "\${words[1]}" in
    ...
    add-mcp)
      ...
      ;;
    upgrade)
      COMPREPLY=( $(compgen -W "-y --yes \${common_options}" -- "\${cur}") )
      ;;
    *)
      COMPREPLY=( $(compgen -W "\${common_options}" -- "\${cur}") )
      ;;
...
_${command}() {
  ...
  case "$words[2]" in
    ...
    add-mcp)
      ...
      ;;
    upgrade)
      _values 'option' '-y' '--yes' ${COMMON_OPTIONS.map((s) => `'${s}'`).join(' ')}
      ;;
    *)
      _describe 'option' common_opts
      ;;
...
complete -c ${command} -n "__fish_seen_subcommand_from upgrade" -s y -l yes -d "跳过确认，直接升级"
