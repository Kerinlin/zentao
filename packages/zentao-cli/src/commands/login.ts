...
import { saveProfile, profileKey, getProfile, getCurrentProfile, buildProfile } from '../config/store.js';
        ...
        .action(async (opts) => {
            ...
            try {
                ...
                if (opts.useEnv) {
                    ...
                    const prompted = await promptLogin(existing ? { url: existing.server, account: existing.account } : undefined);
                    server = prompted.url;
