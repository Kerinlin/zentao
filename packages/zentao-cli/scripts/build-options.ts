...
/** Strip npm scope so binary names stay flat (`@kerin/zentao-cli` → `zentao-cli`). */
export function binaryBaseName(packageName: string): string {
...
export function buildCompileOptions(input: BuildCompileInput): CompileTargetOptions[] {
    ...
    const baseName = binaryBaseName(input.packageName);

    return resolvedTargets.map((target) => ({
        id: target.id,
        bunTarget: target.bunTarget,
        outfile: input.outfile ?? join(input.outdir, `${baseName}-${outputSuffix(target.id)}${windowsExtension(target.id)}`),
    }));
}
