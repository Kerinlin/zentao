...
```bash
zentao bug --product=<产品ID> --browseType=all --orderBy=id_desc --recPerPage=1000 \
  --filter='assignedTo:<当前账号>,status:active' --pick=id,title,severity,pri
```
...
| 看我的 Bug | `zentao bug --product=<id> --orderBy=id_desc --recPerPage=1000 --filter='assignedTo:<账号>,status:active'` |
